...
@defstruct*[execution
  ...
  The @racket[name] names the operator,
    except the special string @racket["adjust"],
...
@defproc[(rival-set-profiling! [machine rival-machine?] [enabled boolean?]) void?]{
  Enables or disables execution profiling for the @racket[machine].
}

@defproc[(rival-profiling-enabled? [machine rival-machine?]) boolean?]{
  Returns whether profiling is currently enabled for the @racket[machine].
}
