...
def plot_repeats_plot(outcomes, args):
    ...
    ax.bar(np.arange(len(baseline)) + 0.9, baseline_percentages, color="green", alpha=1, width=0.5, label='ziv+', hatch='/')
    ...
    iterations = sorted(set(baseline['iter']).union(set(rival['iter'])))
